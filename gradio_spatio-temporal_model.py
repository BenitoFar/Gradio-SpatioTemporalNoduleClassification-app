...
This tool predicts the probability of a lung nodule being malignant using a spatio-temporal model.

### How the Model Works:

#### Inputs: The model can process up to three 3D CT scans of the same lung nodule taken at different timepoints:
...
- You can provide **complete data** (all three timepoints) or **partial data** (one or two timepoints). The model will still generate a prediction using the available information.  
...
with gr.Blocks() as demo:
    ...
    logo_upm = 'C:/Users/benit/Desktop/Script/Python/local/streamlit_demo/nlst/logos/Logo UPM.png'
    logo_ciber = 'C:/Users/benit/Desktop/Script/Python/local/streamlit_demo/nlst/logos/Logo CIBER BBN.jpg'
    logo_bit = 'C:/Users/benit/Desktop/Script/Python/local/streamlit_demo/nlst/logos/bit_logo.png'
    with gr.Row():
    ...
    with gr.Row():
        gr.Markdown(value = summary_text_markdown)
    ...
    classify_button = gr.Button("Classify Nodule Malignancy", size = "lg")
    ...
    with gr.Row():
        box7_output = gr.Image(label=f"GradCAM Centroid Slice T{to_subscript('t-2')}")
        box8_output = gr.Image(label=f"GradCAM Centroid Slice T{to_subscript('t-1')}")
        box9_output = gr.Image(label=f"GradCAM Centroid Slice T{to_subscript('t')}")
        
    # Output components in separate rows
    with gr.Row():
        malignancy_output = gr.Label(label="Lung Nodule Malignancy Probability")
    ...
    gr.Examples(
        examples=[
                    ...
                    [
                        f"{examples_dir}109198_T0_1.2.840.113654.2.55.200179948888767177268464555974180947437.nrrd", 
                        f"{examples_dir}109198_T1_1.2.840.113654.2.55.193477603872374096013600773662943359616.nrrd", 
                        f"{examples_dir}109198_T2_1.2.840.113654.2.55.314248407427914969129482316084647169364.nrrd",
                        "(128, 194, 111)", 
                        "(126, 185, 128)",
                        "(120, 160, 110)", 
                    ],
                    [
                        f"{examples_dir}116176_T0_1.2.840.113654.2.55.151307258625893782678875432347954089352.nrrd", 
                        f"{examples_dir}116176_T1_1.2.840.113654.2.55.66374051382854062645623198899617645924.nrrd", 
                        f"{examples_dir}116176_T2_1.2.840.113654.2.55.151609145404393538590054193612894313530.nrrd",
                        "(433, 269, 149)", 
                        "(427, 256, 75)",
                        "(440, 233, 66)", 
                    ],
                    [
                        f"{examples_dir}122117_T0_1.2.840.113654.2.55.73409874272321877625524264273833606033.nrrd", 
                        f"{examples_dir}122117_T1_1.2.840.113654.2.55.301270571084139063152819655606377756670.nrrd", 
                        f"{examples_dir}122117_T2_1.2.840.113654.2.55.32496072008315898589920724459377473768.nrrd",
                        "(138, 244, 114)", 
                        "(157, 266, 102)",
                        "(160, 258, 104)", 
                    ],
        ...
        inputs=[image1, image2, image3, centroid1, centroid2, centroid3],
        outputs=[malignancy_output, box1_output, box2_output, box3_output],
        fn=spatiotemporal_classifier,
        cache_examples=False,
        label="Preloaded Examples",
    )
    
    #remove all files in the temp folder
    remove_files_from_folder("C:/Users/benit/Desktop/Script/Python/local/streamlit_demo/nlst/tmp")
